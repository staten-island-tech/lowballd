...
<div>
    <div class="flex flex-row">
        ...
        <div class="h-96 w-11/12 mx-auto flex flex-wrap justify-start">
            <UserPost v-for="item in postsArray" :key="item.id" :postImage="item.images[0]" :postDate="item.date.slice(4,-42)"></UserPost>
        </div>
</div>
</template>
...
import UserPost from './profileposts_components/UserPost.vue'
    export default {
        ...
        methods: {
            async callApi() {
            try {
                const response = await fetch("https://lowballd-backend.onrender.com/api/posts", {
                });
